...
###################################
####### Deprecated Section ########
##### Le Great MemAlloc Games #####
############# of 2022 #############
###################################

	# def get_speed(self):
	# 	pos_diff = self.encoder_pos[-1] - self.encoder_pos[-2]
	# 	time_diff = (self.encoder_time[-1] - self.encoder_time[-2]) / 10**9
	# 	self.last_speed = pos_diff/time_diff
	# 	return (pos_diff / time_diff)

	# def get_controller_data(self):
	# 	pos_diff = self.encoder_pos[-1] - self.encoder_pos[-2]
	# 	time_diff = (self.encoder_time[-1] - self.encoder_time[-2]) / 10**9
	# 	return (pos_diff / time_diff), time_diff

	# def __reform_arrays(self):
	# 	for _ in range(len(self.encoder_pos),self.array_size):
	# 		self.encoder_pos.append(0)
	# 		self.encoder_time.append(0)

	# def __trim_arrays(self):
	# 	index = len(self.encoder_pos)
	# 	for i in range(1,index+1):
	# 		if (not (self.encoder_pos[index-i] or self.encoder_time[index-i])):
	# 			self.encoder_pos.pop(index-i)
	# 			self.encoder_time.pop(index-i)

	# def reset_arrays(self):
	# 	for i,_ in enumerate(self.encoder_pos):
	# 		self.encoder_pos[i] = 0
	# 		self.encoder_time[i] = 0

	# def __pull_print_data(self):
	# 	print_data_pos = []
	# 	print_data_time = []
	# 	index = len(self.encoder_pos)
	# 	print(index)
	# 	for i in range(index):
	# 		print(self.encoder_pos[i])
	# 		if (self.encoder_pos[i] and self.encoder_time[i]):
	# 			print_data_pos.append(self.encoder_pos[i])
	# 			print_data_time.append(self.encoder_time[i])
	# 	print(print_data_pos)
	# 	return print_data_pos, print_data_time

	# def print_results_terminal(self):
	# 	self.__trim_arrays()
	# 	for i in range(len(self.encoder_pos)):
	# 		if (self.encoder_pos[i] or self.encoder_time[i]):
	# 			print(self.encoder_pos[i],(self.encoder_time[i] - self.encoder_time[0])/10**9)
	# 	print(len(self.encoder_pos))
	# 	self.__reform_arrays()
